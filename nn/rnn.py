...
class RNNUnit(Module):
    ...
    def forward(self, input, hidden = None):
        ...
        if hidden == None:
            hidden = Tensor(np.zeros((batch_size, self.hidden_size)), requires_grad=True, is_parameter=True)
        return Tanh()(input@self.weight_ih.T() + self.bias_ih + hidden@self.weight_hh.T() + self.bias_hh)
...
class TimeIterator(Module):
    ...
    def forward(self,input,hidden = None):
        ...
        # Creating 't_idx': proper indexing of packedSeq from batch_sizes
        t_idx = [0]
        for i in range(len(batch_sizes)):
            t_idx.append(t_idx[i]+batch_sizes[i])

        packedHidden = []
        lastHidden = []
        for i in range(len(t_idx)-1):
            x = data[t_idx[i]:t_idx[i+1]]
            if hidden != None:
                hidden_prev = hidden[0:x.shape[0]]
                # The fact that some hidden states are dropped -> those dropped hs are the last output of the hidden states 
                # [Note: 'hidden' here is hidden_(t-1) since it's before applying hidden = self.unit(x, hidden_prev)]
                if len(hidden) != len(hidden_prev):
                    num_hidden_dropped = len(hidden)-len(hidden_prev)
                    for i in range(-1,-(num_hidden_dropped+1),-1):
                        if i == -1:
                            lastHidden.append(hidden[i:])
                        else:
                            lastHidden.append(hidden[i:i+1])
                hidden = self.unit(x, hidden_prev)
            else:
                # time = -1
                hidden = self.unit(x, hidden)
            packedHidden.append(hidden)

        # Include last outputs of longest timestep samples
        for i in range(-1,-(len(hidden)+1),-1):
            if i == -1:
                lastHidden.append(hidden[i:])
            else:
                lastHidden.append(hidden[i:i+1])

        packedHidden = tensor.cat(packedHidden)
        # The order in which 'lastHidden' was populated was reversed
        lastHidden.reverse()
        lastHidden = tensor.cat(lastHidden)

        return (PackedSequence(packedHidden, sorted_indices, batch_sizes), lastHidden)
